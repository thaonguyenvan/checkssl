...
@section('content')
	<div class="container mt-1">
		...
		<p style="color: red;">
			Giới hạn domain của bạn: {{$limit_domain}}
		</p>
		<small>
			Lưu ý: Khi thêm domain, mặc định thời gian cảnh báo trước khi hết hạn và thời gian cảnh báo lại là lần lượt {{$limit_default->send_noti_before}} và {{$limit_default->send_noti_after}} ngày. Sau thời gian cảnh báo lại, hệ thống sẽ tự động gửi cảnh báo 1 ngày 1 lần nếu domain chưa được gia hạn. Click vào từng domain để xem thông tin và chỉnh sửa.
		</small>
		...
		<div class="white-bg p-2">
			<div class="row">
				<div class="col-md-12">
					...
					<button type="button" class="btn btn-danger" onclick="delete_domain()">Xóa Domain</button>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="collapse" id="show-add-domain">
						<div class="down-10">
							<form class="form-inline" action="user/adddomain" accept-charset="UTF-8" data-remote="true" method="post">
					...
					<div class="collapse" id="show-bulk-add">
						<div class="down-10">
							<form class="form" action="user/addmultipledomain" accept-charset="UTF-8" method="post">
								<input name="utf8" type="hidden" value="✓">
								<input type="hidden" name="_token" value="{{csrf_token()}}">
					...
					<div class="collapse" id="show-filter">
						<div class="down-10">
							<form class="form" action="user/domain/filter" accept-charset="UTF-8" method="post">
								<input name="utf8" type="hidden" value="✓">
								<input type="hidden" name="_token" value="{{csrf_token()}}">
								<div class="form-row">
						...
						<table class="table table-bordered text-center">
							...
							<tbody>
								...
								<tr>
									<td>
										<div class="checkbox">
									      <input type="checkbox" class="check" value="{{$domain->id}}">
									  	</div>
									</td>
									<td><a href="user/detaildomain/{{$domain->id}}">{{$domain->domain}}</a></td>
									<td>{{date('d-m-Y', strtotime($domain->create_at))}}</td>
									<td>{{date('d-m-Y', strtotime($domain->expire_at))}}</td>
									<td>{{$domain->dayleft." ngày"}}</td>
									<td>{{$domain->owner}}</td>
									...
									<td>{{$domain->send_noti_before." ngày"}}</td>
									<td>{{$domain->send_noti_after." ngày"}}</td>
								</tr>
								@endforeach
							</tbody>
						</table>
